...
export default function NewsletterSection() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!email) return;
    setSubmitted(true);
    toast.success('You\'re subscribed! Check your email for a welcome offer.', { duration: 4000 });
    setEmail('');
  };
      ...
      <div className="max-w-2xl mx-auto px-4 sm:px-6 text-center">
        ...
        {submitted ? (
          <div className="flex items-center justify-center gap-3 bg-green-50 border border-green-200 rounded-2xl px-6 py-5">
            ...
            <p className="text-green-700 font-medium">You're subscribed! Welcome to the StitchPro family.</p>
          </div>
        ) : (
          <form onSubmit={handleSubmit} className="flex gap-3 max-w-md mx-auto">
            ...
            <button
              type="submit"
              className="flex items-center gap-2 bg-gold-500 hover:bg-gold-600 text-navy-900 font-semibold px-5 py-3 rounded-xl transition-colors shadow-gold flex-shrink-0"
            >
